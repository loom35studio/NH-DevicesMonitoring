...
async function check477(vb, printer) {
    try {
        ...
        await db.promise().query("UPDATE black SET pages = ?, percentage = ?, date = ?, changed = ? WHERE printerID = ?", [bk, Math.round((vb[7].value * 100) / vb[6].value), Bk, bkCh, printer.ID]);
        await db.promise().query("UPDATE yellow SET pages = ?, percentage = ?, date = ?, changed = ? WHERE printerID = ?", [ye, Math.round((vb[1].value * 100) / vb[0].value), Ye, yeCh, printer.ID]);
        await db.promise().query("UPDATE magenta SET pages = ?, percentage = ?, date = ?, changed = ? WHERE printerID = ?", [ma, Math.round((vb[3].value * 100) / vb[2].value), Ma, maCh, printer.ID]);
        await db.promise().query("UPDATE cyan SET pages = ?, percentage = ?, date = ?, changed = ? WHERE printerID = ?", [cy, Math.round((vb[5].value * 100) / vb[4].value), Cy, cyCh, printer.ID]);
